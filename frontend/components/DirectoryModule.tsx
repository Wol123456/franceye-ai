            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full overflow-hidden shadow-xl flex flex-col min-h-[600px]">
                ...
                <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                    <button 
                        ...
                        className={`flex-1 py-4 text-sm font-semibold transition ${currentTab === 'brand' ? 'text-blue-800 dark:text-blue-600 border-b-2 border-blue-600 bg-white/80 dark:bg-slate-800/50' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'}`}
                    >
                        Şube Yöneticileri
                    </button>
                    <button 
                        onClick={() => setActiveSettingsTab('admin')} 
                        className={`flex-1 py-4 text-sm font-semibold transition ${currentTab === 'admin' ? 'text-blue-800 dark:text-blue-600 border-b-2 border-blue-600 bg-white/80 dark:bg-slate-800/50' : 'text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200'}`}
                    >
                        Sistem Yöneticileri
                    </button>
                </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                                ...
                                <button 
                                    ...
                                    className="bg-blue-800 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-md flex items-center gap-2"
                                >
                                    Ekle
                                </button>
                            </div>
                            ...
                            {isAdminFormOpen && (
                                <div className="bg-white dark:bg-slate-800 border border-blue-200 dark:border-blue-900/50 rounded-xl p-5 shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <input type="text" placeholder="Ad Soyad" value={newAdmin.name} onChange={e => setNewAdmin({...newAdmin, name: e.target.value})} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 outline-none focus:border-blue-700 text-slate-800 dark:text-white" />
                                        <input type="email" placeholder="E-posta" value={newAdmin.email} onChange={e => setNewAdmin({...newAdmin, email: e.target.value})} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 outline-none focus:border-blue-700 text-slate-800 dark:text-white" />
                                        <input type="text" placeholder="Telefon" value={newAdmin.phone} onChange={e => setNewAdmin({...newAdmin, phone: e.target.value})} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 outline-none focus:border-blue-700 text-slate-800 dark:text-white" />
                                        <input type="text" placeholder="Profil Fotoğrafı URL (Opsiyonel)" value={newAdmin.photo} onChange={e => setNewAdmin({...newAdmin, photo: e.target.value})} className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 outline-none focus:border-blue-700 text-slate-800 dark:text-white" />
                                        <label className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 cursor-pointer hover:border-blue-700 transition">
                                            <input type="checkbox" checked={newAdmin.receive_emails} onChange={e => setNewAdmin({...newAdmin, receive_emails: e.target.checked})} className="w-5 h-5 accent-blue-800 rounded" />
                                    ...
                                    <div className="flex justify-end gap-3 mt-5">
                                        ...
                                        <button onClick={handleAddNewAdmin} className="bg-blue-800 hover:bg-blue-700 text-white px-6 py-2 rounded-lg text-sm font-semibold shadow-md">{editingAdminId ? 'Güncelle' : 'Kaydet'}</button>
                                    </div>
                                </div>
                            )}
                                    ...
                                    <div key={admin.id} className="flex justify-between items-center p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:shadow-md transition">
                                        <div className="flex items-center gap-4">
                                            {admin.photo ? (
                                                ...
                                                <div className="w-12 h-12 rounded-full bg-slate-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-800 dark:text-blue-600 font-bold text-lg">
                                                    {admin.name.charAt(0)}
                                                </div>
                                            )}
                                            <div>
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => handleEditAdmin(admin)} className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-blue-800 dark:text-blue-600 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-600">Düzenle</button>
                        ...
                        <div className="space-y-6">
                            ...
                            {isBranchManagerFormOpen && (
                                <div className="bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-900/50 rounded-xl p-5 shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="flex justify-end gap-3 mt-5">
                                        ...
                                        <button onClick={handleSaveBranchManager} className="bg-blue-800 hover:bg-emerald-700 text-white px-6 py-2 rounded-lg text-sm font-semibold shadow-md">{editingBranchId ? 'Güncelle' : 'Kaydet'}</button>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="space-y-3">
                                {displayList.map((branch: any, i: number) => (
                                    <div key={branch.place_id || i} className={`flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-white dark:bg-slate-800 border ${branch.isSaved ? 'border-emerald-200 dark:border-blue-900' : 'border-slate-200 dark:border-slate-700'} rounded-xl hover:shadow-md transition gap-4`}>
                                        <div className="flex items-center gap-4">
                                            {branch.photo ? (
                                                ...
                                                <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${branch.isSaved ? 'bg-slate-100 dark:bg-emerald-900/30 text-blue-800 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-500'}`}>
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs font-semibold text-blue-800 dark:text-emerald-400 mt-0.5">
                                        ...
                                        <div className="flex gap-2">
                                            <button 
                                                ...
                                                className="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-blue-800 dark:text-emerald-400 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-600"
                                            >
